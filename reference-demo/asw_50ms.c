/**
 * @file    asw_50ms.c
 * @brief   50 ms rate - TASK_CMD: scope channel PD4 + serial command
 *          parser and button events.
 *
 ...
 * line buffer below is rate-local. The button event arrives via kernel
 * IPC (mailbox under RES_DEMO). See asw_signals.h for the concurrency
 * contract.
 */
...
/** Scope jitter channel: PD4 (Nano D4) toggles at 10 Hz. */
...
/**
 * TASK_CMD - 50 ms. Toggles scope channel PD4, then services two inputs:
 *   1. button events arriving via the mailbox (from TASK_BUTTON), the
 *      receive guarded by RES_DEMO (the ISR-ceiling handoff),
...
void Task_Cmd(void)
{
    OsPoolHandleType h = OS_POOL_INVALID_HANDLE;
    StatusType       rx = E_OS_NOFUNC;
    char             c;

    Actuator_Trigger(&actScope);

    /* --- 1. button events (mailbox handoff under RES_DEMO) --------- */
    if (GetResource(RES_DEMO) == E_OK)
    {
        rx = OS_MailboxReceive(&h);
