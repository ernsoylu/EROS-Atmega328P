...
/*   TASK_STATUS     prio 1  wcet <= 2 tick(s)  (500 ms) */
/*   TASK_RAMP       prio 2  wcet <= 1 tick(s)  (100 ms) */
/*   TASK_CMD        prio 3  wcet <= 2 tick(s)  (50 ms) */
/*   TASK_BUTTON     prio 4  wcet <= 1 tick(s)  (10 ms) */
...
#define TASK_BUTTON           ((TaskType)4u)
...
OS_STATIC_ASSERT(
    ((1u << TASK_STARTUP) | (1u << TASK_STATUS) | (1u << TASK_RAMP) | (1u << TASK_CMD) | (1u << TASK_BUTTON)) ==
    ((1u << OS_NUM_TASKS) - 1u),
...
OS_STATIC_ASSERT((RES_UART < OS_NUM_RESOURCES),
