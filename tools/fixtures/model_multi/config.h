...
/*   TASK_MOTOR      prio 1  wcet <= 2 tick(s)  (20 ms) */
/*   TASK_APPKNBSWT  prio 2  wcet <= 2 tick(s)  (10 ms) */
#define OS_NUM_TASKS          3u

#define TASK_INIT             ((TaskType)0u)
#define TASK_MOTOR            ((TaskType)1u)
#define TASK_APPKNBSWT        ((TaskType)2u)

extern void Task_Init(void);
extern void Task_motor(void);
extern void Task_appKnbSwt(void);

/* ---- alarms: one cyclic alarm per periodic task ------------------ */
#define OS_NUM_ALARMS         2u

#define ALARM_APPKNBSWT       ((AlarmType)0u) /* 10 ms -> TASK_APPKNBSWT */
#define ALARM_MOTOR           ((AlarmType)1u) /* 20 ms -> TASK_MOTOR */

/* Period macros for SetRelAlarm/SetAbsAlarm calls in the app. */
#define TASK_APPKNBSWT_PERIOD_TICKS 10u
#define TASK_MOTOR_PERIOD_TICKS 20u

/* ---- resources (IPCP; ceiling = highest-priority user) ---------- */
#define OS_NUM_RESOURCES      1u

#define RES_RTE               ((ResourceType)0u) /* users: APPKNBSWT, MOTOR */
...
#define OS_ALIVE_REQUIRED_MASK \
    ((uint8_t)((1u << TASK_APPKNBSWT) | (1u << TASK_MOTOR)))
...
OS_STATIC_ASSERT(
    ((1u << TASK_INIT) | (1u << TASK_MOTOR) | (1u << TASK_APPKNBSWT)) ==
    ((1u << OS_NUM_TASKS) - 1u),
...
OS_STATIC_ASSERT((ALARM_APPKNBSWT < OS_NUM_ALARMS) && (ALARM_MOTOR < OS_NUM_ALARMS),
